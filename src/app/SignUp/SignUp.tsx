...
export default function THINGY1() {
  ...
  useEffect(() => {
    const plswork = Keyboard.addListener("keyboardDidHide", () => {
      Keyboard.dismiss();
      setFocused1(false);
    });
    return () => {};
      ...
      try {
        ...
        if (
          ...
          const headear = {
            ...
            Accept:
              "text/html,application/xhtml+xml,applicati  on/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8",
          ...
          try {
            if (
              head["set-cookie"]
                .split(";")[4]
                .split(",")[1]
                .includes("TabroomToken=%24")
            ) {
              try {
                await signInWithEmailAndPassword(auth, ewail, paword);
                const thing = await getDoc(
                  doc(db, "users", auth.currentUser.uid),
                );
                console.log(thing.data().Skin);
  ...
  const tingy = useAnimatedStyle(() => {
    return {
      backgroundColor: withTiming(buttton ? "#ffffff" : "#0080ff", {
        duration: 200,
      }),
      borderRadius: withTiming(buttton ? 8 : 16, { duration: 400 }),
      borderWidth: withTiming(buttton ? 2 : 0, { duration: 400 }),
      borderColor: "#0080ff",
      transform: [{ scale: withTiming(buttton ? 0.95 : 1, { duration: 400 }) }],
    };
  });
  const SIGNUP = async () => {
    setbuttton(true);
    ...
    if (email !== "" && password !== "" && email.includes("@")) {
      ...
      try {
        ...
        alert(
          "Failed, your credentials are a littleeeee screwed up, try again",
        );
      }
    } else {
      if (email === "" || password === "") {
    ...
    if (!New_person && logged_in) {
      let response = await fetch(
        "https://us-central1-sandd-1304d.cloudfunctions.net/updatepassword_login",
        {
          method: "POST",
          mode: "cors",
          headers: {
      ...
      if (!thingy_works) {
        alert(
          "something didn't work out for some reason, sooo umm idk, redo it",
        );
        alert(response_now);
      } else {
      ...
      {loading ? (
        ...
        <View style={[styles.other_thing, focused1 && styles.thingg2]}>
          ...
          <TextInput
            ...
            style={[styles.first, focused1 && styles.first2]}
            value={email}
          ...
          <TextInput
            ...
            style={[styles.second, focused1 && styles.second2]}
            value={password}
            onChangeText={setPas}
            secureTextEntry
          />
